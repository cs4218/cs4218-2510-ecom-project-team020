...
AI Declaration: Test files were generated with AI assistance but ALL test cases are refined and validated.

| Domain                           | File                                                | Description                                                                                                                                                                                                                         |
| -------------------------------- | --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Admin Orders Page           | `client/src/pages/admin/AdminOrders.test.js`        | Validates order retrieval, status mutation workflows, and payment success/failure branches. Includes mocks for axios and toast notifications. Achieved full line and branch coverage with race-condition and null-field edge tests. |
| Products Page (Admin)        | `client/src/pages/admin/Products.test.js`           | Ensures product list integrity, correct sort order, resilience to malformed or missing data, consistent slug rendering, and stable UI under simulated API latency/failure.                                                          |
| Users Page (Admin)           | `client/src/pages/admin/Users.test.js`              | Confirms proper table and layout rendering, presence of `Layout` and `AdminMenu` components, and prevention of redundant re-renders; tests keyboard navigation and ARIA focus behavior.                                             |
| User Menu Component          | `client/src/components/UserMenu.test.js`            | Tests route-aware active-state highlighting, keyboard accessibility, and structural link validation; covers all equivalence partitions of navigation contexts.                                                                      |
| Private Route Component      | `client/src/components/Routes/PrivateRoute.test.js` | Simulates missing, valid, and expired JWT tokens; asserts correct redirect logic, cleanup on unmount, and concurrent API call handling.                                                                                             |
| Header Component             | `client/src/components/Header.test.js`              | Verifies dynamic navigation items by auth state, category dropdown population, cart-badge accuracy, and logout clearing behavior under both authenticated and guest modes.                                                          |
| Footer Component             | `client/src/components/Footer.test.js`              | Checks static content, internal/external link correctness, visual separators, copyright-year auto-update, and accessibility semantics.                                                                                              |
| Layout Wrapper               | `client/src/components/Layout.test.js`              | Ensures global layout consistency—`Header`, `Footer`, and `Toaster` present across all routes; validates `<Helmet>` metadata defaults and page-specific overrides.                                                                  |
| Product Controller (Backend) | `controllers/productController.test.js`             | Exhaustively tests CRUD, filtering, pagination, counting, and file-upload boundaries using Equivalence Partitioning and Boundary Value Analysis. Includes schema validation and simulated MongoDB errors.                           |
| User Model (Backend)         | `models/userModel.test.js`                          | Verifies schema constraints, default values, trimming and validation behavior, and persistence integrity across multiple mock documents.                                                                                            |
